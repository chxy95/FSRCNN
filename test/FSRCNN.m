...
for i = 1 : filters 
    ...
    data_tmp(:,:,i) = max(data_tmp(:,:,i) + bias(i), 0) + prelu_conv{1}(i) * min(data_tmp(:,:,i) + bias(i),0);
end
...
for idx = 2 : layer_num-1
    ...
    for i = 1 : filters
        ...
        data_tmp(:,:,i) = max(data_tmp(:,:,i) + bias(i),0) + prelu_conv{idx}(i) * min(data_tmp(:,:,i) + bias(i),0);
    end
